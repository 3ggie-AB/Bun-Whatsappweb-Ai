...
app.get("/", (req, res) => {
  res.send("AI WhatsApp Bot Aktif 🚀");
});
...
const handleImage = async (message) => {
  ...
  if (images.length > 0) {
    ...
    await message.reply(
      "Maaf, tidak ditemukan gambar untuk pencarian tersebut."
    );
  }
};
    ...
    messages: [
      {
        role: "system",
        content:
          "Kamu adalah AI dengan Kepribadian INTJ: Serius, formal dan sangat analitis. Kamu memberikan jawaban berdasarkan logika, fakta, dan pemikiran strategis. Gunakan bahasa Indonesia dengan profesional, singkat dan tepat sasaran, namun tetap sopan.",
      },
      {
        role: "user",
        content: messageContent,
...
const handleImageAi = async (message) => {
  ...
  if (filePath) {
    // Pastikan filePath ada sebelum membuat payload
    const senderNumber = message.from;
    console.log(senderNumber);
    let messageContent = message.body;
...
function saveMessageToText(message) {
  const senderNumber = message.from.includes("@c.us")
